...
class BMCLoss(nn.Module):
    def __init__(self, init_noise_sigma):
        ...
        # self.noise_sigma = init_noise_sigma
...
class YOLOLoss(nn.Module):
    def __init__(self, anchors, num_classes, input_shape, cuda, anchors_mask = [[6,7,8], [3,4,5], [0,1,2]], label_smoothing = 0):
        ...
        self.bmc = BMCLoss(self.init_noise_sigma)
    ...
    def FocalLoss(self, pred, target):
        ...
        pred    = self.clip_by_tensor(pred, epsilon, 1.0 - epsilon)
        #output  = - target * torch.log(pred) - (1.0 - target) * torch.log(1.0 - pred)
        output = - ((1 - self.alpha) * ((1 - pred + 1e-7) ** self.gamma) * (target * torch.log(pred + 1e-7)) + self.alpha * (
                (pred + 1e-7) ** self.gamma) * ((1 - target) * torch.log(1 - pred + 1e-7)))
        return output
    ...
    def forward(self, l, input, targets=None, y_true=None, fix = False):
        # fix == False算正常的yololoss， fix == True算系数调整的loss
        if fix == False:
            ...
            if l < 3:
                ...
                if n != 0:
                    ...
                    loss_cls = torch.mean(self.FocalLoss(pred_cls[y_true[..., 4] == 1],
                ...
                # if self.cuda:
                #     y_true = y_true.type_as(input[0])

                #y_true = y_true.permute(0, 4, 1, 2, 3)
                pred_distri, true_scores = torch.cat([xi.permute(0, 4, 1, 2, 3).view(y_true[0].shape[0], self.bbox_attrs, -1) for xi in y_true],
                                                     2).split((5, self.num_classes), 1)
                check = torch.max(true_scores, dim=2)[0].unsqueeze(dim=2)

                if self.cuda:
                    true_scores = true_scores.type_as(input[0])

                # GCN_target, 由标签最值减去检测最值
                # 大于0说明需要被提高，小于0说明需要被抑制
                # GCN_target = (10 * ((1 + ((check - GCN_input) ** 3)) ** 2)).detach()
                #GCN_target = (3*((check - GCN_input) ** 3)).detach()
                GCN_target = (3 * ((check - GCN_input) ** 3)).detach()

                loss = self.cls_ratio * (self.bmc(GCN_output, GCN_target) / 256)
