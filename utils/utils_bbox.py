...
class DecodeBox():
    ...
    def decode_box_fix(self, inputs, meta):

        # meta = [[[1.5217], [1.5234], [2.0216]]]
        # meta = np.array(meta)
        ...
        # # meta对角化
        # # 对角矩阵化
        # meta = meta.squeeze(axis=2)
        # meta = torch.diag_embed(meta)
        ...
        for i, input in enumerate(inputs):
            ...
            pred_cls    = torch.bmm(meta, pred_cls)
            pred_cls    = pred_cls.permute(0, 2, 1)
            ...
            _scale = torch.Tensor([input_width, input_height, input_width, input_height]).type(FloatTensor)
            output = torch.cat((pred_boxes.view(batch_size, -1, 4) / _scale,
                                conf.view(batch_size, -1, 1), pred_cls), -1)
            outputs.append(output.data)
        return outputs

    # 加法修正
    def decode_box_fix1(self, inputs, meta):

        # meta = meta.permute(0, 2, 1)
        # print(meta)

        # meta = [[[1.0217, 1.0234, 2.0216]]]
        # meta = np.array(meta)
        # meta = torch.from_numpy(meta)
        # meta = meta.cuda()
