...
if __name__ == "__main__":
    ...
    # --------------------------------------------------------------------------------------#
    classes_path = 'model_data/amos_classes.txt'
    # --------------------------------------------------------------------------------------#
    ...
    json_path_val = "/root/autodl-tmp/code_in_lib/yolo5_coco_myGCN3/coco_data/amos_coco/val_annotations.json"
    coco_val = COCO(annotation_file=json_path_val)
    ...
    if map_mode == 0 or map_mode == 1:
        ...
        for image_id in range(1, len(coco_val.imgs) + 1):
            img_name = coco_val.loadImgs(image_id)[0]['file_name']
            image_path = "/root/autodl-tmp/code_in_lib/yolo5_coco_myGCN3/coco_data/amos_coco/JPEGImages/" + img_name
            image = Image.open(image_path)
            if map_vis:
                image.save(os.path.join(map_out_path, "images-optional/" + img_name))
            yolo.get_map_txt(img_name, image, class_names, map_out_path)
    ...
    if map_mode == 0 or map_mode == 2:
        ...
        for image_id in range(1, len(coco_val.imgs) + 1):
            img_name = coco_val.loadImgs(image_id)[0]['file_name']
            with open(os.path.join(map_out_path, "ground-truth/" + img_name + ".txt"), "w") as new_f:
                # root = ET.parse(os.path.join(VOCdevkit_path, "VOC2007/Annotations/" + image_id + ".xml")).getroot()
                # 获取对应图像id的所有标注信息
                ann_ids = coco_val.getAnnIds(imgIds=image_id)

                # 根据标注信息获取所有target
                targets = coco_val.loadAnns(ann_ids)

                # iscrowd为1的标签将一类物体集群归类为一个该物体，会影响画mask，例如所有书架上的书，跳过此类mask
                targets = [x for x in targets if not ('iscrowd' in x and x['iscrowd'])]
                ...
                for target in targets:
                    ...
                    #     if int(difficult) == 1:
                    #         difficult_flag = True
                    obj_name = class_names[target['category_id']]
                    if obj_name not in class_names:
                        continue
                    bndbox = target['bbox']
                    left = bndbox[0]
                    top = bndbox[1]
                    right = bndbox[0] + bndbox[2]
                    bottom = bndbox[1] + bndbox[3]
