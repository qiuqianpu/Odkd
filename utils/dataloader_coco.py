...
class YoloDataset(Dataset):
    def __init__(self, coco, img_path, input_shape, num_classes, anchors, anchors_mask, epoch_length, mosaic, train,
                 mosaic_ratio=0.7):
        ...
        self.epoch_now = -1
        self.length = len(self.coco.dataset['images'])
    ...
    def __getitem__(self, index):
        ...
        if 1 > 2:
            ...
            image, box = self.get_random_data(index, self.img_path, self.input_shape, random=self.train)
    ...
    def get_random_data(self, index, img_path, input_shape, jitter=.3, hue=.1, sat=0.7, val=0.4, random=True):
        if index == 0:
            index = 1
